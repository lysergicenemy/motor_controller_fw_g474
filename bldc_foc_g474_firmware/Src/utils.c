...
float utils_fast_atan2(float y, float x)
{
	...
	if (x >= 0)
	{
		...
		float rsq = r * r;
		angle = ((0.1963f * rsq) - 0.9817f) * r + (MF_PI / 4.0f);
	}
	else
	{
		float r = (x + abs_y) / (abs_y - x);
		float rsq = r * r;
		angle = ((0.1963f * rsq) - 0.9817f) * r + (3.0f * MF_PI / 4.0f);
	}
...
/**
 * Fast cos implementation. (~100 cycles)
 *
 * See http://lab.polygonal.de/?p=205
 *
 * @param angle
